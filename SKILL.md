...
- **進入新組時**，先把 `scenario` 情境句唸給使用者，讓他腦袋進入狀況。
- **然後一個一個技能問**，每題格式：
  > 「『研究』(調查資料、得出結論) — 你覺得是哪一類？
  >  1️⃣ On Fire（我很拿手，每天都想做）
  >  2️⃣ Heating Up（還不夠熟，但想多做）
  >  3️⃣ Burnout（做得來但做久會累）
  >  4️⃣ Cold（不擅長也提不起勁）」
- 使用者回 1/2/3/4 → 立刻問下一題。第一題後簡化提示（只列 1/2/3/4 對應的標籤），免得每題都重複長文。
